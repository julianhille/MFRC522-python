...
class PICC_Type(Enum):
    ...
    def get_name(self):
        ...
        '''
        if self == PICC_Type.PICC_TYPE_ISO_14443_4:
            return 'PICC compliant with ISO/IEC 14443-4'
        elif self == PICC_Type.PICC_TYPE_ISO_18092:
            return 'PICC compliant with ISO/IEC 18092 (NFC)'
        elif self == PICC_Type.PICC_TYPE_MIFARE_MINI:
            return 'MIFARE Mini, 320 bytes'
        elif self == PICC_Type.PICC_TYPE_MIFARE_1K:
            return 'MIFARE 1KB'
        elif self == PICC_Type.PICC_TYPE_MIFARE_4K:
            return 'MIFARE 4KB'
        elif self == PICC_Type.PICC_TYPE_MIFARE_UL:
            return 'MIFARE Ultralight or Ultralight C'
        elif self == PICC_Type.PICC_TYPE_MIFARE_PLUS:
            return 'MIFARE Plus'
        elif self == PICC_Type.PICC_TYPE_MIFARE_DESFIRE:
            return 'MIFARE DESFire'
        elif self == PICC_Type.PICC_TYPE_TNP3XXX:
            return 'MIFARE TNP3XXX'
        elif self == PICC_Type.PICC_TYPE_NOT_COMPLETE:
...
class StatusCode(Enum):
    ...
    STATUS_CANCELED          = 9    # Used in SimpleMFRC522 to signal that blocking read/write was canceled 
    ...
    def get_name(self):
        ...
        if self == StatusCode.STATUS_OK:
            return 'Success.'
        elif self == StatusCode.STATUS_ERROR:
            return 'Error in communication.'
        elif self == StatusCode.STATUS_COLLISION:
            return 'Collission detected.'
        elif self == StatusCode.STATUS_TIMEOUT:
            return 'Timeout in communication.'
        elif self == StatusCode.STATUS_NO_ROOM:
            return 'A buffer is not big enough.'
        elif self == StatusCode.STATUS_INTERNAL_ERROR:
            return 'Internal error in the code. Should not happen.'
        elif self == StatusCode.STATUS_INVALID:
            return 'Invalid argument.'
        elif self == StatusCode.STATUS_CRC_WRONG:
            return 'The CRC_A does not match.'
        elif self == StatusCode.STATUS_MIFARE_NACK:
